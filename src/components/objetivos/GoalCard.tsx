    ...
    <Link href={`/objetivos/${goal.id}`}>
      <div className="bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-2xl p-5 transition">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <span className={`text-xs ml-auto ${daysLeft < 0 ? 'text-red-400' : daysLeft < 7 ? 'text-yellow-400' : 'text-slate-500'}`}>
            {daysLeft < 0 ? 'Vencido' : daysLeft === 0 ? 'Hoy vence' : `${daysLeft} días restantes`}
          </span>
        </div>
      </div>
    </Link>
